...
ending_date="2022-12-31" #IMPORTANT: Historical inflation data on the current year might be unavailable
   ...
   tabsetPanel(               
         ...
         tabPanel("Portfolio Analysis", 
               ...
               mainPanel(
                 ...
                 ), #tab panel end (Portfolio Analysis)
       tabPanel("Indexes", 
            sidebarLayout(
               sidebarPanel(width = 3, 
...
df_inflation$money_input_infl <-input$money_in_portf/length(unique(input$selected_tickers_portf))/df_inflation$in_2020_dollars_factor 
df_inflation$money_input_infl_year <- input$money_in_portf/df_inflation$in_2020_dollars_factor 
       ...
       DCA_df$DCA <- (sum(hist_price_infl_filtered_interactive()$money_input_infl)/length(unique(input$selected_tickers_portf)))/DCA_df$Stocks
        ...
        DCA_df_divs$YoC_Ticker <- ((DCA_df_divs$div_cash_flow)/(DCA_df_divs$invested)+1)*100-100 
          ...
          DCA_df_divs_interactive_v2  <- reactive ({
                                            
        
          DCA_df_divs <- left_join(DCA_df_interactive(), divs_agg %>% group_by(ticker), by=c('ticker'='ticker')) 
          
  
      
          DCA_df_hist <- left_join(DCA_df_divs, df_historical, by=c('ticker'='ticker', 'Year_div'='ref_date_year')) 
          
         
         DCA_df_hist$ticker_dca <- paste('DCA',DCA_df_hist$ticker)
        ...
        # DCA_df_divs$YoC_Ticker <- ((DCA_df_divs$div_cash_flow)/(DCA_df_divs$invested)+1)*100-100 
          ...
          return( 
           ...
           output$DCA_DF_v2 = renderDT( DCA_df_divs_interactive_v2())
      ...
      add_trace(x = ~ticker, y = ~YoC_Ticker, name = 'Portfolio YoC') %>%   
              ...
              # layout(title = paste("YoC TODAY* per ticker with last buy on ",input$years_portf[2]))  %>%
                 ...
                 output$Stocks_bought <- renderPlotly({
                    ...
                    layout(title = "<b>Number of stocks that were bought<b>",
...
# 
#                    plot_ly(DCA_df_divs_interactive_v2() ) %>%
#             add_trace(x = ~Year_div, y = ~DCA, type = 'scatter', mode = "lines", name = ~ticker)%>%
#                         layout(title = '<b> DCA <b>', xaxis = list(title = 'Date'),
#                                  yaxis = list(title = 'Price ($)'), legend = list(title=list(text='<b> Ticker </b>')))
                  
            #       
            #        plot_ly(DCA_df_divs_interactive_v2() ) %>%
            # add_trace(x = ~ref_date, y = ~DCA, type = 'scatter', mode = "lines", name = ~ticker)%>%
            #             layout(title = '<b> DCA <b>', xaxis = list(title = 'Date'),
            #                      yaxis = list(title = 'Price ($)'), legend = list(title=list(text='<b> Ticker </b>')))
        ...
        output$Inflation_Evolution <-renderPlotly({
          ...
          plot_ly(df_inflationinteractive()) %>%

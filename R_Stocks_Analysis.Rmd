...
plot_ly(df_DCA) %>%
      ...
      add_trace(x = ~ref_date, y = ~DCA, type = 'scatter', mode = "lines", yaxis = "y2", name = ~ticker)%>%
...
fig <- fig %>%
    add_trace(data=df_historical, name=~ticker, x = ~ref_date, y =~price_close, type = 'scatter', mode = "lines")

# add data from second dataframe Df2
fig <- fig %>%
    add_trace(data=df_DCA, name=~ticker, x = ~ref_date, y = ~DCA, type = 'scatter', mode = "lines")
